...
from selenium import webdriver
browser = webdriver.Firefox(executable_path="/Users/sid/PycharmProjects/pythonStarter/driver/geckodriver")
browser.get('https://web.whatsapp.com')


def getNews():

    text_box = browser.find_element(By.CLASS_NAME("_2S1VP"))
...
while True:
    unread = browser.find_element(By.CLASS_NAME("OUeyt")) # The green dot tells us that the message is new
    name, message = '', ''
    if len(unread) > 0:
        ele = unread[-1]
        ...
        try:
            ...
            pass
        try:
            name = browser.find_element(By.CLASS_NAME("_2zCDG")).text  # Contact name
            message = browser.find_element(By.CLASS_NAME("vW7d1"))[-1]  # the message content
            if 'activate bot' in message.text.lower():
                if name not in bot_users:
                    bot_users[name] = True
                    text_box = browser.find_element(By.CLASS_NAME("_2S1VP"))
                    response = "Hi " + name + ". AS Bot here :). Now I am activated for you\n"
                    text_box.send_keys(response)
